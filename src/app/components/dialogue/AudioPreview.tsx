...
export function AudioPreview({ result, onError }: AudioPreviewProps) {
  ...
  const mergeAudioSegments = async (result: GenerationResult) => {
    ...
    try {
      ...
      if (retryCount < MAX_RETRIES) {
        console.log(`Retrying audio merge (attempt ${retryCount + 1} of ${MAX_RETRIES})...`)
        setRetryCount(prev => prev + 1)
        setTimeout(() => mergeAudioSegments(result), 1000)
      } else {
        onError(`Failed to merge audio segments after ${MAX_RETRIES} attempts: ${errorMessage}`)
      }
    } finally {
  ...
  return (
    <div className="bg-white rounded-lg border p-6 space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-center p-4 bg-gray-50 rounded-lg">
              ...
              <span className="ml-2 text-gray-600">
                {retryCount > 0 ? `Retrying merge (${retryCount}/${MAX_RETRIES})...` : 'Processing audio...'}
              </span>
            </div>
            {processingStatus && (
            ...
            <div className="p-4 bg-yellow-50 text-yellow-700 rounded-lg">
              {detailedError ? (
                <>
                  ...
                  <p className="mt-1 whitespace-pre-wrap">{detailedError}</p>
                </>
              ) : (
                'Failed to merge audio segments.'
              )}
            </div>
            <button
              onClick={handleRetry}
      ...
      {(!mergedAudioUrl || detailedError) && result.audioUrls.length > 0 && (
        <div className="space-y-4">
